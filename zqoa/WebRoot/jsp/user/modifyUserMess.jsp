					...
					<form action="" method="post" id="Case-form" class="add-form">
						<table>
							...
							<tr>
								...
								<td>
									<input type="text" name="zqUserModel.name" id="caseName" value="<s:property value="userinfo.name"/>"/>
								</td>
							</tr>
							<tr>
								<th>
							...
							<tr>
								...
								<td>
									<select name="zqUserModel.groupId">
										...
										<option value="<s:property value="id"/>" <s:if test="userinfo.groupId==id">selected="selected"</s:if>>
		...
		if(confirm("请检查所填数据,是否确认?")){
			...
			$.ajax({
				...
				success : function(data) { //成功后返回的页面
					...
					alert(data.info);
					if (data.status == '1') {
						window.location.href="listUser.htm";
					}
				},
