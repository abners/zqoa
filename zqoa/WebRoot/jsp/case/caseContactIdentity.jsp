		...
		<div id="main">
			<div class="toolbar">
				...
				<div id="toolbar-content">
					<p class="crumb">
						...
						<a href="caseContactIdentity.htm">案件联系人身份管理</a>
					</p>
			...
			<div class="wrap">
				<fieldset class="forms-area">
					...
					<div class="list-form">
						<ec:table items="contactIdentityList" var="row"
							retrieveRowsCallback="org.extremecomponents.table.callback.LimitCallback"
							action="caseContactIdentity.htm" width="90%" autoIncludeParameters="false">

							<ec:row highlightRow="true">

								<ec:column property="identityName" style="height:35px;" title="案件联系人身份">
								<span style="display:inline;" >${row.identityName}</span>
								<span class="hidden-field"><input class="edit-box"
										...
										<input class="btn-edit-cate ui-button" type="button"
											...
											name="identityId" value="${row.identityId}" /> </span>
								</ec:column>
								<ec:column property="cust_typeValue" escapeAutoFormat="true"
									style="text-align:center;height:35px;" title="操作">
									<a href="javascript:void(0);" title="修改">修改</a>|<a href="${row.identityId}" title="删除">删除</a>
								</ec:column>
						...
						<form id="typeForm" action="" method="post">
							<div class="cate-info">
								...
								<input type="text" id="identityName" name="zqContactidentityModel.identityName" />
		...
		function(){
			...
			$("input[type=button][value=提交]").click(function(){
				...
				if(confirm("是否确认修改?")){
					$.ajax({
						type : "post",
						url : "modifyCaseIdentity.htm",
						dataType : "json",
						data : {"zqContactidentityModel.identityName":typeName,
							"zqContactidentityModel.identityId":$(this).parent("span").find("input[name=identityId]").val()
						},
						success : function(data) { //成功后返回的页面
							alert(data.info);
							if (data.status == '1'){
			...
			$("a[title=删除]").click(function(){
				...
				if(confirm("删除后将无法恢复，是否确认?")){
					$.ajax({
						type : "post",
						url : "deltCaseIdentity.htm",
						dataType : "json",
						data : {"identityId":contractTypeId
						},
						success : function(data) { //成功后返回的页面
							alert(data.info);
							if (data.status == '1'){
