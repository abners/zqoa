			...
			<div class="toolbar">
				...
				<div id="toolbar-content">
					<p class="crumb">
						...
						<a href="javascript:void(0);">添加案件</a>
					</p>
					...
					<form action="" method="post" id="Case-form" class="add-form">
						<table>
							...
							<tr>
								...
								<td>
									<font id="custName" color="red"></font>
									<input type="hidden" name="zqCaseModel.custId" id="custId" value="" />
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									<select name="zqCaseModel.contId" id="contId" style="width:300px;">
									<option value="-1">--请选择--</option>
										<s:iterator value="cont">
										<option value="<s:property value="id"/>">
											<s:property value="contName"/>
										</option>
										</s:iterator>
									</select>
								</td>
...
$(document).ready(function(){
	$("#contId").change(function(){
		...
		if(contId=="-1"){
			...
			if($.trim($("#"+contId).val())=='' || $.trim($("#"+contId).val())=="null"){
				$("#custName").attr({color:"red"});
				$("#custName").text("此合同尚无客户!");
				//设置客户id
				$("#custId").val("");
			}else{
				$("#custName").attr({color:"black"});
				$("#custName").text($("#"+contId).val());
				//设置客户id
				$("#custId").val($("#"+contId+"s").val());
			}
		}
	...
	$("#btn-add-Case").click(function(){
		...
		if($("#custId").val()==''){
			alert("所选合同尚无客户，请为该合同添加客户后再进行操作!");
			return ;
		}
